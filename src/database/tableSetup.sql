...
CREATE TABLE queue (Id INT PRIMARY KEY AUTO_INCREMENT, contact_Id INT, json LONGTEXT, INDEX con_ind (contact_id), FOREIGN KEY (contact_id) REFERENCES contact(Id) ON DELETE CASCADE) ENGINE=INNODB;
CREATE TABLE IF NOT EXISTS login(Id INT PRIMARY AUTO_INCREMENT, Username VARCHAR(255), Password VARCHAR(255)) ENGINE=InnoDB;